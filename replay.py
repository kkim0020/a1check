...
class ActionInfo:
    def __init__(self, action, undo):
        self.action  : PaintAction = action
        self.undo : bool= undo
...
class ReplayTracker:
    ...
    def add_action(self, action: PaintAction, is_undo: bool=False) -> None:
        ...
        """
        self.actions.append(ActionInfo(action, is_undo))
    ...
    def play_next_action(self, grid: Grid) -> bool:
        ...
        """
        try:
            action_info: ActionInfo = self.actions.serve()
        ...
        action: PaintAction = action_info.action
        is_undo = action_info.undo
